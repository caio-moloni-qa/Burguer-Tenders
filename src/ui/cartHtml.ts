...
export function renderCartLines(cart: CartStore): string {
  ...
  if (lines.length === 0) {
    return `<p class="cart-drawer__empty">${t("cartEmpty")}</p>`;
  }
      ...
      .map((line) => {
        ...
        <li class="cart-line" data-product-id="${escapeHtml(line.productId)}">
          <div class="cart-line__info">
            ...
            <span class="cart-line__each">${formatPrice(product.priceUsd)} ${t("cartEach")}</span>
          </div>
          <div class="cart-line__controls">
            ...
            <button type="button" class="cart-line__remove" data-action="remove-line" data-product-id="${escapeHtml(line.productId)}">${t("cartRemove")}</button>
          </div>
          <div class="cart-line__sub" data-testid="line-total">${formatPrice(lineTotal)}</div>
        </li>`;
      })
      .join("")}
  </ul>`;
